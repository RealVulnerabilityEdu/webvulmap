...
from ka import KnowledgeArea, KnowledgeUnit, KnowledgeTopic, LearningOutcome, KnowledgeUnitTier
...
State = Enum('State',
             ['IN_KA',
              'KU_HEADING', 'KU_HOURS', 'KU_TOPICS_HEADING', 'TOPICS_TIER', 'TOPICS_LIST',
...
def get_outcome_line(line, f, line_stack):
    while True:
        ...
        if not next_line: return line
        if pattern_page_number.match(next_line): continue
        if not pattern_ku_partial_outcomes_line.match(next_line) and not pattern_ku_heading.match(next_line):
            if next_line.strip():
...
def parse_outcome_line(line):
    ...
    if len(matches.groups()) != 2: return None
...
def parse_ku_title_line(line):
    ...
    if not matches: return None
    if matches.groups() != 1: return None
...
def parse_ku_tier_line(line):
    ...
    if len(m_list) % 3 == 0:
        ...
        for i in range(0, len(m_list), 3):
            tier_list.append(KnowledgeUnitTier(m_list[i+1], m_list[i+2]))
        return tier_list
    elif len(m_list) == 1:
...
def parse_topic_line(line):
    ...
    if not matches: return None
    if len(matches.groups()) != 1: return None
    return matches.group(1).strip()
...
def parse_ka(ka_fn):
    ...
    with open(ka_fn, mode='rt') as f:
        ...
        while line_stack:
            ...
            if '12. Apply consistent documentation and program style standards that contribute to the readability and' in line:
                pass
            ...
            elif state == State.KU_TOPICS_HEADING and not pattern_topics_tier.match(line):
                state = State.TOPICS_TIER
                topics_tier = None
                line_stack.append(line)
            elif (state == State.KU_TOPICS_HEADING or state == State.TOPICS_TIER) and pattern_ku_topic_line.match(line):
                state = State.TOPICS_LIST
                topics_list = []
                line_stack.append(line)
            elif state == State.TOPICS_LIST and pattern_ku_topic_line.match(line):
                topic = parse_topic_line(line)
                topics_list.append(KnowledgeTopic(topic, topics_tier))
            elif state == State.TOPICS_LIST and pattern_ku_subtopic_line.match(line):
                subtopic = parse_subtopic_line(line)
                topics_list[-1].add_subtopic(subtopic)
            elif state == State.TOPICS_LIST and pattern_ku_outcomes_heading.match(line):
                state = State.LEARNING_OUTCOMES_HEADING
            elif state == State.LEARNING_OUTCOMES_HEADING and pattern_ku_partial_outcomes_line.match(line):
            ...
            elif state == State.LEARNING_OUTCOMES_HEADING and pattern_ku_heading.match(line):
                state = State.KU_HEADING
                ku_title = parse_ku_title_line(line)
                ku = KnowledgeUnit(ku_title)
                ka.add_ku(ku)
